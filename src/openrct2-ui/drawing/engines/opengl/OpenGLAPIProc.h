...
// OPENGL_PROC_OPT: like OPENGL_PROC but does not abort init if the proc address is null.
// Use for GL 3.0+ functions that are never called on a 2.1 context.
#ifndef OPENGL_PROC_OPT
    #define OPENGL_PROC_OPT OPENGL_PROC
#endif
...
OPENGL_PROC_OPT(PFNGLBINDFRAGDATALOCATIONPROC, glBindFragDataLocation)
...
OPENGL_PROC_OPT(PFNGLCLEARBUFFERFVPROC, glClearBufferfv)
OPENGL_PROC_OPT(PFNGLCLEARBUFFERUIVPROC, glClearBufferuiv)
...
OPENGL_PROC_OPT(PFNGLUNIFORM1UIPROC, glUniform1ui);
OPENGL_PROC_OPT(PFNGLUNIFORM1UIVPROC, glUniform1uiv);
